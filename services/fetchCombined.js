...
export async function fetchCombined(token, translationId, script = 'uthmani', chapterId = null) {
  ...
  const fetchOne = async (id) => {
    ...
    fs.writeFileSync(path.join(chapterDir, `combined_${translationId}.json`), JSON.stringify({ id, name, verses }, null, 2), 'utf-8');
    log(`💾 Saved combined ${translationId} for surah ${id} (${script})`);
  };
